...
//decrypt text based off key
void decryptMessage(char *key, char *text, char **msg){
   char *alpha = "ABCDEFGHIJKLMNOPQRSTUVWXYZ ";
   char *msgTemp = malloc(sizeof(char)*strlen(text+1));	//temp for decrypted msg
   memset(msgTemp, '\0', strlen(text+1));
   *msg = malloc(sizeof(char)*strlen(text+1));	//this will be our decrypted msg (+1 for null later)
   memset(*msg, '\0', strlen(text+1));	//init to all null characters
   int *temp1 = malloc(sizeof(int)*strlen(text)+1);
   int *temp2 = malloc(sizeof(int)*strlen(text)+1);	//same length
   int *temp3 = malloc(sizeof(int)*strlen(text)+1);	//to store the encrypted msg
   int i, j;


   //store ciphertext as its integer value (from alpha array) into temp1
